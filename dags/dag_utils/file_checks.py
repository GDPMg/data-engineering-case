...
def check_entity_silver(domain: str, entity: str, run_date: str) -> bool:
    base = os.environ.get("PIPELINE_BASE_DIR", "/opt/pipeline")
    path = os.path.join(base, "data", "silver", domain, entity, run_date)
    files = glob.glob(os.path.join(path, "*.csv"))
    if files:
        logger.info(f"Silver data found for '{entity}' ({run_date}): {[os.path.basename(f) for f in files]}")
        return True
    logger.warning(f"No silver data for '{entity}' at {path} — pipeline will be skipped")
    return False
